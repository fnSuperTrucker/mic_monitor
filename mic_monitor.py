...
def callback(indata, frames, time, status):
    global vu_meter, input_status_indicator
    try:
        if status:
            print(f"Audio input status: {status}")
        if any(indata):
            volume_norm = np.abs(indata).mean() * 50  # Adjust scaling as needed
...
try:
    with sd.InputStream(device=device, channels=channels, samplerate=samplerate, blocksize=blocksize, callback=callback):
        root = tk.Tk()
